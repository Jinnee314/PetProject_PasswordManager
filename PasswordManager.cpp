...
// Получаем флаг для того, чтобы не копировать весь data
void PasswordManager::createStringFromData(bool moving)
{
	fileData.clear();

	// Если можно переместить данные, то используем перемещение.
	// Выбрал небольшое дублирование кода для копирования только строк записи 
	// вместо копирования всего map.
	if (moving)
	{
...
void PasswordManager::saveDataInFile(bool endWork)
{
	...
	{
		// Если данные в дальнейшем не понадобятся, то можем строки записей
		// из data переместить в строку fileData.
		// Если понадобятся, то необходимо копировать строки записей в строку fileData
