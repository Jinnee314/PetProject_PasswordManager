...
private:
	...
	// Флаг показывает, можем мы перемещать данные или они нам ещё понадобятся в дальнейшем
	// и их стоит копировать.
	void createStringFromData(bool moving);
...
public:
	...
	// Используем метод saveDataInFile для сохранения изменений при окончании работы
	// программы.
	~PasswordManager();
	...
	// Зашифровываем данные и записываем их в файл fileStorage.
	...
	// не делает.
	// Флаг показывает, понадобятся ли нам данные после выполнения этой функции.
	void saveDataInFile(bool endWork);
